...
- after withholding the next accepted short shards, temperature/register scraps,
  and inner slogans, rebuilt and hot-redeployed at `20260926T200614+0300`;
...
The next live pressure pass on the deployed `-n 80` binary
(`arianna_live_probe_runs/20260913T213602+0300`) confirmed the old explicit
mid-cut signatures were gone and the boundary still blocked the previous
diagnostic strings. It also exposed the broader diagnostic meter shape
`I sense ... at 0.` on the human surface. The live membrane now withholds
...
admission final-gate receipt:
`arianna.live_route_weighted_admission_resonance_graft_admission_final_gate_observation_boundary_preflight_gate_candidate_store_reader_proof_precondition_decision_promotion_switch_enable_gate_live_stage_final_gate.v1`.
It consumes and verifies the live-stage seal receipt, carries source seal,
authority, permit, readiness, ledger, writer, live-stage, enable-gate, switch,
promotion, decision, precondition/proof/reader/store/candidate/gate
provenance, and keeps the final gate closed:
`admission_final_gate_state=blocked`,
`admission_final_gate_action=reject_blocked_admission_seal`,
`admission_final_gate_target=live_admission_final_gate`,
`admission_final_gate_target_kind=weighted_internal_world_shadow_graft_admission_final_gate_observation_boundary_preflight_gate_candidate_store_reader_proof_precondition_decision_promotion_switch_enable_gate_live_stage_seal`,
`admission_final_gate_target_mode=closed_final_gate_guard_dry_run`,
`admission_final_gate_dry_run_only=true`,
`admission_final_gate_seal_verified=false`,
`admission_final_gate_authority_verified=false`,
`admission_final_gate_permit_verified=false`,
`admission_final_gate_ledger_verified=false`, and
`admission_final_gate_ready=false`.

This is a final-gate guard only. It consumes the immutable seal as proof that
the authority door stayed shut; it does not grant authority, open admission, or
mutate the body. It preserves `ledger_contract=none`,
`ledger_entrypoint=none`, `ledger_receipt_shape=none`,
`ledger_write_scope=none`, `ledger_ready=false`,
`ledger_append_allowed=false`, `contracts_ready=false`,
`write_allowed=false`, `admission_allowed=false`,
`live_admission_enabled=false`, `mutates_state=false`,
`body_mutation_allowed=false`, and `authority_granted=false`. Added
metabolism CLI entrypoints, Makefile aliases, shell wrappers, smoke and
consumer-smoke, plus Go producer/assert tests. The new smoke scripts keep
scratch report/log names short for macOS path limits. Verification: `bash -n`
on the new scripts, full `go test ./... -count=1` in `golib`, and full
weighted live-stage-final-gate consumer smoke with shared GGUF weights all
pass.
...
admission-permit receipt:
`arianna.live_route_weighted_admission_resonance_graft_admission_final_gate_observation_boundary_preflight_gate_candidate_store_reader_proof_precondition_decision_promotion_switch_enable_gate_live_stage_permit.v1`.
It consumes and verifies the readiness receipt, carries source readiness,
...
`writer_state=blocked`, `writer_action=reject_blocked_admission_readiness`,
`rollback_state=blocked`,
`rollback_action=reject_blocked_admission_readiness`,
`ledger_state=blocked`, and
`ledger_action=reject_blocked_admission_readiness`.

This is an admission-permit guard only. It proves that readiness does not imply
manual authority: `admission_permit_state=blocked`,
`admission_permit_action=reject_blocked_admission_readiness`,
`admission_permit_target=live_admission`,
`admission_permit_dry_run_only=true`,
`admission_permit_readiness_verified=false`,
`admission_permit_ledger_verified=false`,
`admission_permit_writer_ready=false`,
`admission_permit_rollback_ready=false`,
`admission_permit_ledger_ready=false`,
`admission_permit_ready=false`, `manual_permit_requested=false`, and
`permit_key_matched=false`. It preserves `ledger_contract=none`,
`ledger_entrypoint=none`, `ledger_receipt_shape=none`,
`ledger_write_scope=none`, `ledger_ready=false`,
`ledger_append_allowed=false`, `contracts_ready=false`,
`write_allowed=false`, `admission_allowed=false`,
`live_admission_enabled=false`, `mutates_state=false`, and
`body_mutation_allowed=false`; all source readiness, ledger-verification,
persistence, implementation, ledger, and upstream writer/live-stage guards stay
...
Verification: `bash -n` on the new scripts, targeted `go test -run 'Permit'`,
full `go test ./... -count=1` in `golib`, `make all nano chorus metabolism kk
doe_field harvest_delta doe_qmatvec_test`, full weighted live-stage-permit
consumer smoke with shared GGUF weights, and `git diff --check` all pass.
...
`arianna.live_route_weighted_admission_resonance_graft_admission_final_gate_observation_boundary_preflight_gate_candidate_store_reader_proof_precondition_decision_promotion_switch_enable_gate_live_stage_ledger_verification.v1`.
It consumes and verifies the ledger-persistence receipt, carries source
ledger-persistence, ledger-implementation, admission-ledger, writer-contract,
writer-inventory, writer-preflight, live-stage, enable-gate, switch,
promotion, decision, precondition/proof/reader/store/candidate/gate
provenance, and keeps `writer_state=blocked`,
`writer_action=reject_blocked_ledger_persistence`,
`rollback_state=blocked`, `rollback_action=reject_blocked_ledger_persistence`,
`ledger_state=blocked`, and
`ledger_action=reject_blocked_ledger_persistence`.

This is a ledger-verification guard only. It proves that read-back verification
does not open persistence: `ledger_verification_state=blocked`,
`ledger_verification_action=reject_blocked_ledger_persistence`,
`ledger_verification_target=admission_ledger_receipt`,
`ledger_verification_receipt_shape=none`,
`ledger_verification_append_only=false`,
`ledger_verification_dry_run_only=true`,
`ledger_verification_receipt_read_back=false`,
`ledger_verification_receipt_verified=false`, and
`ledger_verification_ready=false`. It preserves `ledger_contract=none`,
`ledger_entrypoint=none`, `ledger_receipt_shape=none`,
`ledger_write_scope=none`, `ledger_ready=false`,
`ledger_append_allowed=false`, `contracts_ready=false`,
`write_allowed=false`, `admission_allowed=false`,
`live_admission_enabled=false`, `mutates_state=false`, and
`body_mutation_allowed=false`; all source persistence, implementation, ledger,
and upstream writer/live-stage guards stay closed. Added metabolism CLI
entrypoints, Makefile aliases, shell wrappers, smoke and consumer-smoke, plus
Go producer/assert tests.
Verification: `bash -n` on the new scripts, targeted `go test -run
'LedgerVerification'`, full `go test ./... -count=1` in `golib`, `make all
nano chorus metabolism kk doe_field harvest_delta doe_qmatvec_test`, full
weighted live-stage-ledger-verification consumer smoke with shared GGUF
weights, and `git diff --check` all pass.

## 2026-08-30 - Weighted admission final-gate observation boundary preflight gate candidate store reader proof precondition decision promotion switch enable gate live stage ledger persistence

The blocked live-stage ledger-implementation receipt now feeds a closed
ledger-persistence receipt:
`arianna.live_route_weighted_admission_resonance_graft_admission_final_gate_observation_boundary_preflight_gate_candidate_store_reader_proof_precondition_decision_promotion_switch_enable_gate_live_stage_ledger_persistence.v1`.
It consumes and verifies the ledger-implementation receipt, carries source
ledger-implementation, admission-ledger, writer-contract, writer-inventory,
writer-preflight, live-stage, enable-gate, switch, promotion, decision,
precondition/proof/reader/store/candidate/gate provenance, and keeps
`writer_state=blocked`, `writer_action=reject_blocked_ledger_implementation`,
`rollback_state=blocked`, `rollback_action=reject_blocked_ledger_implementation`,
`ledger_state=blocked`, and
`ledger_action=reject_blocked_ledger_implementation`.

This is a ledger-persistence guard only. It proves the append remains closed:
`ledger_persistence_state=blocked`,
`ledger_persistence_action=reject_blocked_ledger_implementation`,
`ledger_persistence_target=admission_ledger_receipt`,
`ledger_persistence_receipt_shape=none`,
`ledger_persistence_write_scope=none`,
`ledger_persistence_append_only=false`,
`ledger_persistence_receipt_persisted=false`, and
`ledger_persistence_ready=false`. It preserves `ledger_contract=none`,
`ledger_entrypoint=none`, `ledger_receipt_shape=none`,
`ledger_write_scope=none`, `ledger_ready=false`,
`ledger_append_allowed=false`, `contracts_ready=false`,
`write_allowed=false`, `admission_allowed=false`,
`live_admission_enabled=false`, `mutates_state=false`, and
`body_mutation_allowed=false`; all source ledger-implementation, source
admission-ledger, and upstream writer/live-stage guards stay closed. Added
metabolism CLI entrypoints, Makefile aliases, shell wrappers, smoke and
consumer-smoke, plus Go producer/assert tests.
Verification: `bash -n` on the new scripts, targeted `go test -run
'LedgerPersistence'`, full `go test ./... -count=1` in `golib`, `make all
nano chorus metabolism kk doe_field harvest_delta doe_qmatvec_test`, full
weighted live-stage-ledger-persistence consumer smoke with shared GGUF weights,
and `git diff --check` all pass.

## 2026-08-26 - Weighted admission final-gate observation boundary preflight gate candidate store reader proof precondition decision promotion switch enable gate live stage ledger implementation

The blocked live-stage admission-ledger now feeds a closed ledger-implementation
receipt:
`arianna.live_route_weighted_admission_resonance_graft_admission_final_gate_observation_boundary_preflight_gate_candidate_store_reader_proof_precondition_decision_promotion_switch_enable_gate_live_stage_ledger_implementation.v1`.
It consumes and verifies the admission-ledger receipt, carries source ledger,
writer-contract, writer-inventory, writer-preflight, live-stage, enable-gate,
switch, promotion, decision, precondition/proof/reader/store/candidate/gate
provenance, and keeps `writer_state=blocked`,
`writer_action=reject_blocked_admission_ledger`,
`rollback_state=blocked`, `rollback_action=reject_blocked_admission_ledger`,
`ledger_state=blocked`, and
`ledger_action=reject_blocked_admission_ledger`.

This is a ledger-implementation guard only. It keeps the append implementation
closed: `ledger_implementation_state=blocked`,
`ledger_implementation_action=reject_blocked_admission_ledger`,
`ledger_implementation_entrypoint=none`,
`ledger_implementation_receipt_shape=none`,
`ledger_implementation_write_scope=none`,
`ledger_implementation_append_only=false`,
`ledger_implementation_receipt_persisted=false`, and
`ledger_implementation_ready=false`. It preserves
`ledger_contract=none`, `ledger_entrypoint=none`,
`ledger_receipt_shape=none`, `ledger_write_scope=none`,
`ledger_ready=false`, `ledger_append_allowed=false`,
`contracts_ready=false`, `write_allowed=false`,
`admission_allowed=false`, `live_admission_enabled=false`,
`mutates_state=false`, and `body_mutation_allowed=false`; all source ledger
and upstream writer/live-stage guards stay closed. Added metabolism CLI
entrypoints, Makefile aliases, shell wrappers, smoke and consumer-smoke, plus
Go producer/assert tests.
Verification: `bash -n` on the new scripts, targeted `go test -run
'LedgerImplementation'`, full `go test ./...` in `golib`, `make all nano
chorus metabolism kk doe_field harvest_delta doe_qmatvec_test`, full weighted
live-stage-ledger-implementation consumer smoke with shared GGUF weights, and
`git diff --check` all pass.

## 2026-08-26 - Weighted admission final-gate observation boundary preflight gate candidate store reader proof precondition decision promotion switch enable gate live stage ledger

The blocked live-stage writer-contract now feeds a closed admission-ledger
receipt:
`arianna.live_route_weighted_admission_resonance_graft_admission_final_gate_observation_boundary_preflight_gate_candidate_store_reader_proof_precondition_decision_promotion_switch_enable_gate_live_stage_ledger.v1`.
It consumes and verifies the writer-contract receipt, carries source
writer-contract, writer-inventory, writer-preflight, live-stage, enable-gate,
switch, promotion, decision, precondition/proof/reader/store/candidate/gate
provenance, and keeps `writer_state=blocked`,
`writer_action=reject_blocked_writer_contract`,
`rollback_state=blocked`, `rollback_action=reject_blocked_writer_contract`,
...
`ledger_state=blocked`, and
`ledger_action=reject_blocked_writer_contract`.

This is an admission-ledger guard only. It formalizes that no ledger append
contract exists yet: `ledger_contract=none`, `ledger_entrypoint=none`,
`ledger_receipt_shape=none`, `ledger_write_scope=none`,
`ledger_ready=false`, and `ledger_append_allowed=false`. It preserves
`writer_contract=none`, `rollback_contract=none`,
`admission_ledger_contract=none`, `contracts_ready=false`,
`write_allowed=false`, `admission_allowed=false`,
`live_admission_enabled=false`, `mutates_state=false`, and
`body_mutation_allowed=false`; source writer-contract/inventory/preflight/live
stage and upstream enable/switch/promotion guards stay closed. Added
metabolism CLI entrypoints, Makefile aliases, shell wrappers, smoke and
consumer-smoke, plus Go producer/assert tests. The ledger stage is
`post_preflight_gate_candidate_store_reader_proof_precondition_decision_promotion_switch_enable_gate_live_stage_writer_contract_pre_ledger_append`.
Verification: `bash -n` on the new scripts, targeted `go test -run
'LiveStageLedger'`, full `go test ./...` in `golib`, `make all nano chorus
metabolism kk doe_field harvest_delta doe_qmatvec_test`, full weighted
live-stage-ledger consumer smoke with shared GGUF weights, and `git diff
--check` all pass.
...
`arianna.live_route_weighted_admission_resonance_graft_admission_final_gate_observation_boundary_preflight_gate_candidate_store_reader_proof_precondition_decision_promotion.v1`.
It consumes the decision, verifies decision hash/read-back identity, carries the
precondition/proof plus reader/store/candidate/gate provenance, and emits
`promotion=pending_live_admission` as the named next state before any live
admission lane can open.

This is still only a promotion receipt, not authority. The receipt preserves
`ledger_append_allowed=false`, `graft_allowed=false`, `read_only=true`,
`replay_only=true`, `write_allowed=false`, `admission_allowed=false`,
`live_admission_enabled=false`, `mutates_state=false`, and
`body_mutation_allowed=false`; source decision guards remain closed as well.
Added metabolism CLI entrypoints, Makefile aliases, shell wrappers, smoke and
consumer-smoke, plus Go producer/assert tests.
Verification: `bash -n` on the new scripts, targeted `go test -run
'DecisionPromotion'`, full `go test` in `golib`, `make all nano chorus
metabolism kk doe_field harvest_delta doe_qmatvec_test`, full weighted
decision-promotion consumer smoke with shared GGUF weights, and `git diff
--check` all pass.

## 2026-08-24 - Weighted admission final-gate observation boundary preflight gate candidate store reader proof precondition decision

The reader-proof precondition now feeds a closed shadow decision receipt:
`arianna.live_route_weighted_admission_resonance_graft_admission_final_gate_observation_boundary_preflight_gate_candidate_store_reader_proof_precondition_decision.v1`.
It consumes the precondition, verifies its hash/read-back identity, carries the
source proof plus reader/store/candidate/gate provenance, and emits
`decision=shadow_ready` while still blocking any later live admission step until
the decision receipt exists.

This is still not authority. The receipt preserves `ledger_append_allowed=false`,
`graft_allowed=false`, `read_only=true`, `replay_only=true`,
`write_allowed=false`, `admission_allowed=false`,
`live_admission_enabled=false`, `mutates_state=false`, and
`body_mutation_allowed=false`; source precondition/proof guards remain closed as
well. Added metabolism CLI entrypoints, Makefile aliases, shell wrappers, smoke
and consumer-smoke, plus Go producer/assert tests.
Verification: `bash -n` on the new scripts, targeted `go test -run
'ProofPreconditionDecision'`, full `go test` in `golib`, `make all nano chorus
metabolism kk doe_field harvest_delta doe_qmatvec_test`, full weighted
proof-precondition-decision consumer smoke with shared GGUF weights, and
`git diff --check` all pass.

## 2026-08-24 - Weighted admission final-gate observation boundary preflight gate candidate store reader proof precondition

The reader-proof receipt now has a closed precondition consumer:
`arianna.live_route_weighted_admission_resonance_graft_admission_final_gate_observation_boundary_preflight_gate_candidate_store_reader_proof_precondition.v1`.
It consumes the reader proof, carries proof/read-back identity plus the
reader/store/candidate/gate provenance, and blocks the live admission next step
until this precondition receipt exists.

The receipt does not open authority: `ledger_append_allowed=false`,
`graft_allowed=false`, `read_only=true`, `replay_only=true`,
`write_allowed=false`, `admission_allowed=false`,
`live_admission_enabled=false`, `mutates_state=false`, and
`body_mutation_allowed=false`. The source proof's live/body/write guards remain
closed as well. Added metabolism CLI entrypoints, Makefile aliases, shell
wrappers, smoke and consumer-smoke, plus Go producer/assert tests.
Verification: `bash -n` on the new scripts, targeted `go test -run
'PreflightGateCandidateStoreReaderProofPrecondition'`, full `go test` in
`golib`, `make all nano chorus metabolism kk doe_field harvest_delta
doe_qmatvec_test`, full weighted proof-precondition consumer smoke with shared
GGUF weights, and `git diff --check` all pass.

## 2026-08-24 - Weighted admission final-gate observation boundary preflight gate candidate store reader proof

The closed candidate-store reader now feeds a proof receipt:
`arianna.live_route_weighted_admission_resonance_graft_admission_final_gate_observation_boundary_preflight_gate_candidate_store_reader_proof.v1`.
It consumes the reader receipt, revalidates reader/store/candidate/gate/
preflight/boundary/observation provenance, carries reader/store hash and
read-back proofs, and blocks the next step until this proof exists.

This remains a proof-only layer: `ledger_append_allowed=false`,
`graft_allowed=false`, `read_only=true`, `replay_only=true`,
`write_allowed=false`, `admission_allowed=false`,
`live_admission_enabled=false`, `mutates_state=false`, and
`body_mutation_allowed=false`. Added metabolism CLI entrypoints, Makefile
aliases, shell wrappers, smoke and consumer-smoke, plus Go producer/assert
tests. Verification: `bash -n` on the new scripts, targeted `go test -run
'PreflightGateCandidateStoreReaderProof'`, full `go test` in `golib`, `make
all nano chorus metabolism kk doe_field harvest_delta doe_qmatvec_test`, full
weighted reader-proof consumer smoke with shared GGUF weights, and `git diff
--check` all pass.
...
two enabled receipt stages share a name, even if both boundary projections match.
The shell assertion mirrors that consumer contract by rejecting duplicated
expected stage names in fixture reports, keeping report-stage lookup
unambiguous before more admission tooling consumes the compact surface.
...
writing it when no receipt stages were checked, and it has a final report-level
`Passed` guard after the per-stage mismatch diagnostics. Unit coverage now
asserts that empty and disabled-only report inputs fail closed with
`no_receipts_checked`, so the root verdict is enforced in runtime code as well
as in the shell fixture.
...
**Follow-up, same day - persisted admission ledger receipts now have a read-back verification layer.**
`AM_LIVE_ROUTE_TURN_CANDIDATE_ADMISSION_LEDGER_VERIFICATION_DRY_RUN=1` adds
`arianna.live_route_turn_candidate_admission_ledger_verification.v1` after the ledger-persistence boundary. It consumes
only a passed `ledger_persistence_state=ledger_receipt_persisted_dry_run`, rechecks the stable
`ledger-persistence-<hash>` id plus the source `ledger-implementation-<hash>`, `admission-ledger-<hash>`,
`rollback-implementation-<hash>`, and `writer-receipt-<hash>` ids, preserves the full candidate contract provenance, and
reads the persisted receipt back as a non-mutating verification proof.

The passed ledger verification reports `ledger_verification_state=ledger_receipt_verified_dry_run`,
`ledger_verification_action=verify_persisted_admission_ledger_receipt_dry_run`,
`ledger_verification_target=admission_ledger`, `ledger_verification_target_kind=dream_candidate_admission`,
`ledger_verification_target_mode=append_only_dry_run`,
`ledger_verification_receipt_shape=candidate_contract_provenance`, `ledger_verification_append_only=true`,
`ledger_verification_dry_run_only=true`, `ledger_verification_receipt_read_back=true`,
`ledger_verification_receipt_verified=true`, `ledger_verification_ready=true`, `ledger_persistence_ready=true`,
`writer_implementation_ready=true`, `rollback_implementation_ready=true`, `ledger_implementation_ready=true`,
`contracts_ready=false`, `write_allowed=false`, `admission_allowed=false`, `live_admission_enabled=false`,
`body_target=none`, `mutates_state=false`, and a stable `ledger-verification-<hash>` id. Failed persistence receipts,
tampered persistence ids, mismatched source ids, missing provenance, pre-enabled contracts, or any upstream body-write
...
**Follow-up, 2026-08-15 - weighted admission now gates the Resonance graft preflight.**
`arianna.live_route_weighted_admission_resonance_graft_gate.v1` consumes the weighted graft preflight and turns it into
the next closed shadow-graft gate without opening the body. The receipt carries
`status=shadow_graft_gate_ready_dry_run`, `target=resonance`,
`target_kind=weighted_internal_world_shadow_graft_gate`, `target_mode=receipt_only_closed_gate_dry_run`,
`action=gate_weighted_resonance_shadow_graft_dry_run`,
`receipt_shape=weighted_resonance_shadow_graft_gate_contract`, `gate_kind=shadow_graft_gate`,
`gate_mode=no_mutation_gate`, `gate_stage=pre_live_graft_gate`, causal/gate/read-back hashes, and a weighted
graft-gate ID. It revalidates and carries the source preflight, boundary, observation, receiver, intent, final-gate,
seal, permit, authority, contract, precondition, and readiness chain while keeping `graft_allowed=false`,
`raw_dream_text_allowed=false`, `janus_surface_allowed=false`, `cooc_learning_allowed=false`,
`delta_harvest_allowed=false`, `body_mutation_allowed=false`, `live_admission_enabled=false`, and
...
**Follow-up, 2026-08-16 - weighted admission now stores the Resonance graft candidate.**
`arianna.live_route_weighted_admission_resonance_graft_candidate_store.v1` consumes the weighted graft candidate and
turns it into the next closed append-only/read-back store receipt without opening the body. The receipt carries
`status=shadow_graft_candidate_stored_dry_run`, `target=resonance`,
`target_kind=weighted_internal_world_shadow_graft_candidate_store`,
`target_mode=append_only_read_back_store_dry_run`,
`action=store_weighted_resonance_shadow_graft_candidate_dry_run`,
`receipt_shape=weighted_resonance_shadow_graft_candidate_store_receipt`,
`store_kind=shadow_graft_candidate_store`, `store_mode=append_only_read_back_store`,
`store_stage=pre_live_graft_candidate_store`, causal/store/read-back hashes, and a weighted graft-candidate-store ID.
It revalidates and carries the source candidate, gate, preflight, boundary, observation, receiver, intent, final-gate,
seal, permit, and authority chain while keeping `graft_allowed=false`, `raw_dream_text_allowed=false`,
`janus_surface_allowed=false`, `cooc_learning_allowed=false`, `delta_harvest_allowed=false`,
`body_mutation_allowed=false`, `live_admission_enabled=false`, and `mutates_state=false`. `make
...
**Follow-up, 2026-08-17 - weighted admission now consumes the Resonance graft admission proof as a precondition.**
`arianna.live_route_weighted_admission_resonance_graft_admission_proof_precondition.v1` consumes the weighted
graft-admission-proof receipt and turns it into the next closed precondition receipt before any live-route admission
step can rely on the graft proof. The receipt carries
`status=shadow_graft_admission_proof_precondition_satisfied_dry_run`,
`target=live_route_admission_next_step`,
`target_kind=weighted_internal_world_shadow_graft_admission_proof_precondition`,
`target_mode=closed_receipt_precondition_dry_run`,
`action=consume_weighted_resonance_shadow_graft_admission_proof_before_live_route_admission`,
`receipt_shape=weighted_resonance_shadow_graft_admission_proof_precondition_receipt`,
`precondition_kind=shadow_graft_admission_proof_precondition`,
`precondition_mode=closed_receipt_consumption`,
`precondition_stage=pre_live_graft_admission_proof_precondition`, causal/precondition/read-back hashes, and a
weighted graft-admission-proof-precondition ID. It revalidates and carries the source proof, reader, store,
candidate, gate, preflight, boundary, observation, receiver, intent, final-gate, seal, permit, and authority chain
while keeping `graft_allowed=false`, `raw_dream_text_allowed=false`, `janus_surface_allowed=false`,
`cooc_learning_allowed=false`, `delta_harvest_allowed=false`, `body_mutation_allowed=false`,
`live_admission_enabled=false`, and `mutates_state=false`. `make
admission-weighted-admission-resonance-graft-admission-proof-precondition-consumer` proves producer ->
precondition -> contract -> authority -> permit -> seal -> final gate -> resonance intent -> resonance receiver ->
observation -> graft boundary -> graft preflight -> graft gate -> graft candidate -> graft candidate store -> graft
candidate store reader -> graft admission proof -> graft admission proof precondition -> assert.

**Follow-up, 2026-08-18 - weighted admission now decides the Resonance graft admission from the proof precondition.**
`arianna.live_route_weighted_admission_resonance_graft_admission_decision.v1` consumes the weighted
graft-admission-proof-precondition receipt and turns it into the next closed shadow decision receipt. The receipt
carries `status=shadow_graft_admission_decision_ready_dry_run`,
`target=live_route_admission_next_step`,
`target_kind=weighted_internal_world_shadow_graft_admission_decision`,
`target_mode=closed_decision_receipt_dry_run`,
`action=decide_weighted_resonance_shadow_graft_admission_dry_run`, `decision=shadow_ready`,
`receipt_shape=weighted_resonance_shadow_graft_admission_decision_receipt`,
`decision_kind=shadow_graft_admission_decision`, `decision_mode=closed_precondition_decision`,
`decision_stage=pre_live_graft_admission_decision`, causal/decision/read-back hashes, and a weighted
graft-admission-decision ID. It revalidates and carries the source proof precondition, proof, reader, store,
candidate, gate, preflight, boundary, observation, receiver, intent, final-gate, seal, permit, and authority chain
while keeping `graft_allowed=false`, `raw_dream_text_allowed=false`, `janus_surface_allowed=false`,
`cooc_learning_allowed=false`, `delta_harvest_allowed=false`, `body_mutation_allowed=false`,
`admission_allowed=false`, `live_admission_enabled=false`, and `mutates_state=false`. `make
admission-weighted-admission-resonance-graft-admission-decision-consumer` proves producer -> precondition ->
contract -> authority -> permit -> seal -> final gate -> resonance intent -> resonance receiver -> observation ->
graft boundary -> graft preflight -> graft gate -> graft candidate -> graft candidate store -> graft candidate store
...
**Follow-up, 2026-08-18 - weighted admission now holds the Resonance graft admission promotion at a disabled switch.**
`arianna.live_route_weighted_admission_resonance_graft_admission_switch.v1` consumes the weighted
graft-admission-promotion receipt and turns it into a closed switch guard receipt. The receipt carries
`status=shadow_graft_admission_switch_disabled_dry_run`,
`target=live_route_admission_next_step`,
`target_kind=weighted_internal_world_shadow_graft_admission_switch`,
`target_mode=closed_switch_guard_dry_run`,
...
`receipt_shape=weighted_resonance_shadow_graft_admission_switch_receipt`,
`switch_kind=shadow_graft_admission_switch`, `switch_mode=closed_promotion_switch_guard`,
`switch_stage=pre_live_graft_admission_switch`, causal/switch/read-back hashes, and a weighted
graft-admission-switch ID. It revalidates and carries the source promotion, decision, proof precondition, proof,
reader, store, candidate, gate, preflight, boundary, observation, receiver, intent, final-gate, seal, permit, and
authority chain while keeping `graft_allowed=false`, `raw_dream_text_allowed=false`, `janus_surface_allowed=false`,
`cooc_learning_allowed=false`, `delta_harvest_allowed=false`, `body_mutation_allowed=false`,
`admission_allowed=false`, `live_admission_enabled=false`, `write_allowed=false`, and `mutates_state=false`.
`make admission-weighted-admission-resonance-graft-admission-switch-consumer` proves producer -> precondition ->
contract -> authority -> permit -> seal -> final gate -> resonance intent -> resonance receiver -> observation ->
graft boundary -> graft preflight -> graft gate -> graft candidate -> graft candidate store -> graft candidate store
reader -> graft admission proof -> graft admission proof precondition -> graft admission decision -> graft admission
...
**Follow-up, 2026-08-20 - weighted admission now blocks the Resonance graft admission writer preflight.**
`arianna.live_route_weighted_admission_resonance_graft_admission_writer_preflight.v1` consumes the blocked
weighted graft-admission-live-stage receipt and emits a closed writer-preflight receipt. The receipt carries
`status=shadow_graft_admission_writer_preflight_blocked_dry_run`,
`target=live_route_admission_next_step`,
`target_kind=weighted_internal_world_shadow_graft_admission_writer_preflight`,
`target_mode=closed_writer_preflight_guard_dry_run`,
`action=block_weighted_resonance_shadow_graft_admission_live_stage_blocked_dry_run`,
`writer_state=blocked`, `writer_action=reject_blocked_live_stage`,
`rollback_state=blocked`, `rollback_action=reject_blocked_live_stage`,
`stage_state=blocked`, `stage_action=reject_disabled_enable_gate`,
`enable_state=disabled`, `enable_action=require_operator_key`,
`switch_state=disabled`, `switch_action=hold_pending_live_admission`, `promotion=pending_live_admission`,
`receipt_shape=weighted_resonance_shadow_graft_admission_writer_preflight_receipt`,
`writer_preflight_kind=shadow_graft_admission_writer_preflight`,
`writer_preflight_mode=closed_live_stage_writer_preflight_guard`,
`writer_preflight_stage=pre_writer_inventory_graft_admission_writer_preflight`, causal/writer-preflight/read-back
hashes, and a weighted graft-admission-writer-preflight ID. It revalidates and carries the source live stage plus
the source enable gate, switch, promotion, decision, proof precondition, proof, reader, store, candidate, gate,
preflight, boundary, observation, receiver, intent, final-gate, seal, permit, and authority chain while keeping
`requires_writer=true`, `writer_ready=false`, `rollback_required=true`, `requires_rollback=true`,
`rollback_ready=false`, `graft_allowed=false`, `body_mutation_allowed=false`, `admission_allowed=false`,
`live_admission_enabled=false`, `write_allowed=false`, `body_target=none`, and `mutates_state=false`.
`make admission-weighted-admission-resonance-graft-admission-writer-preflight-consumer` proves producer ->
precondition -> contract -> authority -> permit -> seal -> final gate -> resonance intent -> resonance receiver ->
observation -> graft boundary -> graft preflight -> graft gate -> graft candidate -> graft candidate store ->
graft candidate store reader -> graft admission proof -> graft admission proof precondition -> graft admission
decision -> graft admission promotion -> graft admission switch -> graft admission enable gate -> graft admission
live stage -> graft admission writer preflight -> assert.

**Follow-up, 2026-08-20 - weighted admission now blocks the Resonance graft admission writer inventory.**
`arianna.live_route_weighted_admission_resonance_graft_admission_writer_inventory.v1` consumes the blocked
weighted graft-admission-writer-preflight receipt and emits a closed writer-inventory receipt. The receipt carries
`status=shadow_graft_admission_writer_inventory_blocked_dry_run`,
`target=live_route_admission_next_step`,
`target_kind=weighted_internal_world_shadow_graft_admission_writer_inventory`,
`target_mode=closed_writer_inventory_guard_dry_run`,
...
`receipt_shape=weighted_resonance_shadow_graft_admission_writer_inventory_receipt`,
`writer_inventory_kind=shadow_graft_admission_writer_inventory`,
`writer_inventory_mode=closed_writer_preflight_inventory_guard`,
`writer_inventory_stage=pre_writer_contract_graft_admission_writer_inventory`, causal/writer-inventory/read-back
hashes, and a weighted graft-admission-writer-inventory ID. It revalidates and carries the source writer
preflight plus the source live stage, enable gate, switch, promotion, decision, proof precondition, proof,
...
`mutates_state=false`.
`make admission-weighted-admission-resonance-graft-admission-writer-inventory-consumer` proves producer ->
precondition -> contract -> authority -> permit -> seal -> final gate -> resonance intent -> resonance receiver ->
observation -> graft boundary -> graft preflight -> graft gate -> graft candidate -> graft candidate store ->
graft candidate store reader -> graft admission proof -> graft admission proof precondition -> graft admission
decision -> graft admission promotion -> graft admission switch -> graft admission enable gate -> graft admission
...
**Follow-up, 2026-08-20 - weighted admission now blocks the Resonance graft admission ledger.**
`arianna.live_route_weighted_admission_resonance_graft_admission_ledger.v1` consumes the blocked
weighted graft-admission-writer-contract receipt and emits a closed admission-ledger receipt. The receipt carries
`status=shadow_graft_admission_ledger_blocked_dry_run`,
`target=live_route_admission_next_step`,
`target_kind=weighted_internal_world_shadow_graft_admission_ledger`,
`target_mode=closed_admission_ledger_guard_dry_run`,
`action=block_weighted_resonance_shadow_graft_admission_writer_contract_blocked_dry_run`,
`writer_state=blocked`, `writer_action=reject_blocked_writer_contract`,
`rollback_state=blocked`, `rollback_action=reject_blocked_writer_contract`,
`inventory_state=blocked`, `inventory_action=reject_blocked_writer_preflight`,
`contract_state=blocked`, `contract_action=reject_blocked_writer_inventory`,
`ledger_state=blocked`, `ledger_action=reject_blocked_writer_contract`,
`writer_contract=none`, `rollback_contract=none`, `admission_ledger_contract=none`,
`writer_contract_shape=none`, `rollback_contract_shape=none`, `ledger_contract_shape=none`,
...
`writer_contract_present=false`, `rollback_contract_present=false`, `ledger_contract_present=false`,
`contracts_ready=false`,
`receipt_shape=weighted_resonance_shadow_graft_admission_ledger_receipt`,
`admission_ledger_kind=shadow_graft_admission_ledger`,
`admission_ledger_mode=closed_writer_contract_ledger_guard`,
`admission_ledger_stage=pre_ledger_append_graft_admission_ledger`, causal/ledger/read-back hashes, and a weighted
graft-admission-ledger ID. It revalidates and carries the source writer contract plus the source writer inventory,
writer preflight, live stage, enable gate, switch, promotion, decision, proof precondition, proof, reader, store,
candidate, gate, preflight, boundary, observation, receiver, intent, final-gate, seal, permit, and authority chain
while keeping `requires_writer=true`, `writer_ready=false`, `rollback_required=true`, `requires_rollback=true`,
`rollback_ready=false`, `graft_allowed=false`, `body_mutation_allowed=false`, `admission_allowed=false`,
`live_admission_enabled=false`, `write_allowed=false`, `body_target=none`, and `mutates_state=false`.
...
**Follow-up, 2026-08-21 - weighted admission now blocks Resonance graft admission ledger verification.**
`arianna.live_route_weighted_admission_resonance_graft_admission_ledger_verification.v1` consumes the blocked
weighted graft-admission-ledger-persistence receipt and emits a closed ledger-verification receipt. The receipt carries
`status=shadow_graft_admission_ledger_verification_blocked_dry_run`,
`target=live_route_admission_next_step`,
`target_kind=weighted_internal_world_shadow_graft_admission_ledger_verification`,
`target_mode=closed_ledger_verification_guard_dry_run`,
`action=block_weighted_resonance_shadow_graft_admission_ledger_persistence_blocked_dry_run`,
`writer_action=reject_blocked_ledger_persistence`,
`rollback_action=reject_blocked_ledger_persistence`,
`ledger_state=blocked`, `ledger_action=reject_blocked_ledger_persistence`,
`ledger_contract=none`, `ledger_entrypoint=none`, `ledger_receipt_shape=none`,
`ledger_write_scope=none`, `ledger_ready=false`, `ledger_append_allowed=false`,
`ledger_verification_state=blocked`, `ledger_verification_action=reject_blocked_ledger_persistence`,
`ledger_verification_target=admission_ledger_receipt`,
`ledger_verification_target_kind=weighted_internal_world_shadow_graft_admission_ledger_persistence`,
`ledger_verification_target_mode=closed_read_back_guard_dry_run`,
`ledger_verification_receipt_shape=none`, `ledger_verification_append_only=false`,
`ledger_verification_dry_run_only=true`, `ledger_verification_receipt_read_back=false`,
`ledger_verification_receipt_verified=false`, `ledger_verification_ready=false`,
causal/ledger-verification/read-back hashes, and a weighted graft-admission-ledger-verification ID. It revalidates
and carries the source ledger persistence, source ledger implementation, source ledger, source writer contract,
...
**Follow-up, 2026-08-21 - weighted admission now blocks Resonance graft admission authority.**
`arianna.live_route_weighted_admission_resonance_graft_admission_authority.v1` consumes the blocked
weighted graft-admission-permit receipt and emits a closed admission-authority receipt. The receipt carries
`status=shadow_graft_admission_authority_blocked_dry_run`, `target=live_route_admission_next_step`,
`target_kind=weighted_internal_world_shadow_graft_admission_authority`,
`target_mode=closed_authority_guard_dry_run`,
`action=block_weighted_resonance_shadow_graft_admission_permit_blocked_dry_run`,
`writer_action=reject_blocked_admission_permit`, `rollback_action=reject_blocked_admission_permit`,
`ledger_state=blocked`, `ledger_action=reject_blocked_admission_permit`,
`ledger_contract=none`, `ledger_entrypoint=none`, `ledger_receipt_shape=none`,
`ledger_write_scope=none`, `ledger_ready=false`, `ledger_append_allowed=false`,
`admission_authority_state=blocked`, `admission_authority_action=reject_blocked_admission_permit`,
`admission_authority_target=live_admission_authority`,
`admission_authority_target_kind=weighted_internal_world_shadow_graft_admission_permit`,
`admission_authority_target_mode=closed_authority_guard_dry_run`, `admission_authority_dry_run_only=true`,
`admission_authority_permit_verified=false`, `admission_authority_ledger_verified=false`,
`admission_authority_writer_ready=false`, `admission_authority_rollback_ready=false`,
`admission_authority_ready=false`, `admission_authority_granted=false`, `authority_granted=false`,
causal/authority/read-back hashes, and a weighted graft-admission-authority ID. It revalidates and carries the source
...
receiver, intent, final-gate, seal, permit, and authority chain while keeping `contracts_ready=false`,
...
**Follow-up, 2026-08-22 - weighted admission now blocks Resonance graft admission final gate.**
`arianna.live_route_weighted_admission_resonance_graft_admission_final_gate.v1` consumes the blocked
weighted graft-admission-seal receipt and emits a closed admission-final-gate receipt. The receipt carries
`status=shadow_graft_admission_final_gate_blocked_dry_run`, `target=live_route_admission_next_step`,
`target_kind=weighted_internal_world_shadow_graft_admission_final_gate`,
`target_mode=closed_final_gate_guard_dry_run`,
`action=block_weighted_resonance_shadow_graft_admission_seal_blocked_dry_run`,
`writer_action=reject_blocked_admission_seal`, `rollback_action=reject_blocked_admission_seal`,
`ledger_state=blocked`, `ledger_action=reject_blocked_admission_seal`, `ledger_contract=none`,
`ledger_entrypoint=none`, `ledger_receipt_shape=none`, `ledger_write_scope=none`,
`ledger_ready=false`, `ledger_append_allowed=false`, `admission_final_gate_state=blocked`,
`admission_final_gate_action=reject_blocked_admission_seal`,
`admission_final_gate_target=live_admission_final_gate`,
`admission_final_gate_target_kind=weighted_internal_world_shadow_graft_admission_seal`,
`admission_final_gate_target_mode=closed_final_gate_guard_dry_run`,
`admission_final_gate_dry_run_only=true`, `admission_final_gate_seal_verified=false`,
`admission_final_gate_authority_verified=false`, `admission_final_gate_permit_verified=false`,
`admission_final_gate_ledger_verified=false`, `admission_final_gate_ready=false`, causal/final-gate/read-back
hashes, and a weighted graft-admission-final-gate ID. It revalidates and carries the source seal, source
authority, source permit, source readiness, source ledger verification, source ledger persistence, source ledger
implementation, source ledger, source writer contract, source writer inventory, writer preflight, live stage,
enable gate, switch, promotion, decision, proof precondition, proof, reader, store, candidate, gate, preflight,
boundary, observation, receiver, intent, final-gate, seal, permit, and authority chain while keeping
`authority_granted=false`, `contracts_ready=false`, `write_allowed=false`, `admission_allowed=false`,
`live_admission_enabled=false`, `body_mutation_allowed=false`, `body_target=none`, and `mutates_state=false`.
`make admission-weighted-admission-resonance-graft-admission-final-gate-consumer` proves producer ->
writer contract -> ledger -> ledger implementation -> ledger persistence -> ledger verification -> readiness ->
permit -> authority -> seal -> final-gate -> assert.

**Follow-up, 2026-08-22 - weighted admission now drafts Resonance graft admission final-gate intent.**
`arianna.live_route_weighted_admission_resonance_graft_admission_final_gate_intent.v1` consumes the blocked
weighted graft-admission-final-gate receipt and emits a closed final-gate-intent receipt. The receipt carries
`status=shadow_graft_admission_final_gate_intent_blocked_dry_run`, `target=live_route_admission_next_step`,
`target_kind=weighted_internal_world_shadow_graft_admission_final_gate_intent`,
`target_mode=bounded_intent_guard_dry_run`,
`action=draft_weighted_resonance_shadow_graft_admission_final_gate_intent_dry_run`,
`writer_action=reject_blocked_admission_final_gate_intent`,
`rollback_action=reject_blocked_admission_final_gate_intent`, `ledger_state=blocked`,
`ledger_action=reject_blocked_admission_final_gate_intent`, `ledger_contract=none`,
`ledger_entrypoint=none`, `ledger_receipt_shape=none`, `ledger_write_scope=none`,
`ledger_ready=false`, `ledger_append_allowed=false`, `admission_final_gate_intent_state=blocked`,
`admission_final_gate_intent_action=draft_blocked_final_gate_intent`,
`admission_final_gate_intent_target=resonance`,
`admission_final_gate_intent_target_kind=weighted_internal_world_shadow_graft_admission_final_gate`,
`admission_final_gate_intent_target_mode=bounded_intent_guard_dry_run`,
`admission_final_gate_intent_dry_run_only=true`,
`admission_final_gate_intent_final_gate_verified=false`,
`admission_final_gate_intent_seal_verified=false`, `admission_final_gate_intent_ready=false`,
`final_gate_intent_receiver=resonance`, `final_gate_intent_receiver_kind=internal_world`,
`final_gate_intent_influence_kind=bounded_direction`, `final_gate_intent_raw_dream_text_allowed=false`,
`final_gate_intent_janus_surface_allowed=false`, `final_gate_intent_cooc_learning_allowed=false`,
`final_gate_intent_delta_harvest_allowed=false`, `final_gate_intent_pre_state_hash_required=true`,
`final_gate_intent_post_state_hash_required=true`, causal/intent/read-back hashes, and a weighted
graft-admission-final-gate-intent ID. It revalidates and carries the source final gate, source seal, source
authority, source permit, source readiness, source ledger verification, source ledger persistence, source ledger
implementation, source ledger, source writer contract, source writer inventory, writer preflight, live stage,
enable gate, switch, promotion, decision, proof precondition, proof, reader, store, candidate, gate, preflight,
boundary, observation, receiver, intent, final-gate, seal, permit, and authority chain while keeping
`authority_granted=false`, `contracts_ready=false`, `write_allowed=false`, `admission_allowed=false`,
`live_admission_enabled=false`, `body_mutation_allowed=false`, `body_target=none`, and `mutates_state=false`.
`make admission-weighted-admission-resonance-graft-admission-final-gate-intent-consumer` proves producer ->
writer contract -> ledger -> ledger implementation -> ledger persistence -> ledger verification -> readiness ->
permit -> authority -> seal -> final-gate -> final-gate-intent -> assert.

**Follow-up, 2026-08-22 - weighted admission now previews Resonance graft admission final-gate receiver.**
`arianna.live_route_weighted_admission_resonance_graft_admission_final_gate_receiver.v1` consumes the blocked
weighted graft-admission-final-gate-intent receipt and emits a closed final-gate-receiver receipt. The receipt
carries `status=shadow_graft_admission_final_gate_receiver_previewed_dry_run`,
`target=live_route_admission_next_step`,
`target_kind=weighted_internal_world_shadow_graft_admission_final_gate_receiver`,
`target_mode=bounded_receiver_preview_dry_run`,
`action=preview_weighted_resonance_shadow_graft_admission_final_gate_receiver_dry_run`,
`writer_action=reject_blocked_admission_final_gate_receiver`,
`rollback_action=reject_blocked_admission_final_gate_receiver`, `ledger_state=blocked`,
`ledger_action=reject_blocked_admission_final_gate_receiver`, `ledger_contract=none`,
`ledger_entrypoint=none`, `ledger_receipt_shape=none`, `ledger_write_scope=none`,
`ledger_ready=false`, `ledger_append_allowed=false`, `admission_final_gate_receiver_state=previewed`,
`admission_final_gate_receiver_action=preview_blocked_final_gate_receiver`,
`admission_final_gate_receiver_target=resonance`,
`admission_final_gate_receiver_target_kind=weighted_internal_world_shadow_graft_admission_final_gate_intent`,
`admission_final_gate_receiver_target_mode=bounded_receiver_preview_dry_run`,
`admission_final_gate_receiver_dry_run_only=true`,
`admission_final_gate_receiver_intent_verified=false`,
`admission_final_gate_receiver_final_gate_verified=false`, `admission_final_gate_receiver_ready=false`,
`final_gate_receiver=resonance`, `final_gate_receiver_kind=internal_world`,
`final_gate_receiver_influence_kind=bounded_direction`,
`final_gate_receiver_state_hash_mode=blocked_intent_receiver_preview`,
`final_gate_receiver_raw_dream_text_observed=false`,
`final_gate_receiver_raw_dream_text_forwarded=false`,
`final_gate_receiver_raw_dream_text_allowed=false`, `final_gate_receiver_janus_surface_allowed=false`,
`final_gate_receiver_cooc_learning_allowed=false`, `final_gate_receiver_delta_harvest_allowed=false`,
`final_gate_receiver_body_mutation_allowed=false`, receiver pre/post/delta hashes, and a weighted
graft-admission-final-gate-receiver ID. It revalidates and carries the source final-gate intent, source final
gate, source seal, source authority, source permit, source readiness, source ledger verification, source ledger
persistence, source ledger implementation, source ledger, source writer contract, source writer inventory, writer
preflight, live stage, enable gate, switch, promotion, decision, proof precondition, proof, reader, store,
candidate, gate, preflight, boundary, observation, receiver, intent, final-gate, seal, permit, and authority chain
while keeping `authority_granted=false`, `contracts_ready=false`, `write_allowed=false`,
...
**Follow-up, 2026-08-24 - weighted admission now declares Resonance graft admission final-gate observation boundary.**
`arianna.live_route_weighted_admission_resonance_graft_admission_final_gate_observation_boundary.v1` consumes the
recorded weighted graft-admission-final-gate-observation receipt and emits a closed final-gate-observation-boundary
receipt. The receipt carries `status=shadow_graft_admission_final_gate_observation_boundary_declared_dry_run`,
`target=live_route_admission_next_step`,
`target_kind=weighted_internal_world_shadow_graft_admission_final_gate_observation_boundary`,
`target_mode=receipt_only_closed_dry_run`,
`action=declare_weighted_resonance_shadow_graft_admission_final_gate_observation_boundary_dry_run`,
`writer_action=reject_blocked_admission_final_gate_observation_boundary`,
`rollback_action=reject_blocked_admission_final_gate_observation_boundary`, `ledger_state=blocked`,
`ledger_action=reject_blocked_admission_final_gate_observation_boundary`, `ledger_contract=none`,
`ledger_entrypoint=none`, `ledger_receipt_shape=none`, `ledger_write_scope=none`,
`ledger_ready=false`, `ledger_append_allowed=false`,
`admission_final_gate_observation_boundary_state=declared`,
`admission_final_gate_observation_boundary_action=declare_blocked_final_gate_observation_boundary`,
`admission_final_gate_observation_boundary_target=resonance`,
`admission_final_gate_observation_boundary_target_kind=weighted_internal_world_shadow_graft_admission_final_gate_observation`,
`admission_final_gate_observation_boundary_target_mode=receipt_only_closed_dry_run`,
`admission_final_gate_observation_boundary_dry_run_only=true`,
`admission_final_gate_observation_boundary_observation_verified=true`,
`admission_final_gate_observation_boundary_read_back_verified=true`,
`admission_final_gate_observation_boundary_ready=false`,
`final_gate_observation_boundary_kind=blocked_final_gate_observation_boundary`,
`final_gate_observation_boundary_mode=no_mutation_closed_boundary_receipt`,
`final_gate_observation_boundary_stage=post_observation_pre_live_admission`,
raw dream text, Janus surface, cooc learning, delta harvest, and body mutation boundary flags closed,
boundary/read-back hashes, and a weighted graft-admission-final-gate-observation-boundary ID. It revalidates and
carries the source final-gate observation, source final-gate receiver, source final-gate intent, source final gate,
source seal, source authority, source permit, source readiness, source ledger verification, source ledger
persistence, source ledger implementation, source ledger, source writer contract, source writer inventory, writer
preflight, live stage, enable gate, switch, promotion, decision, proof precondition, proof, reader, store,
candidate, gate, preflight, boundary, observation, receiver, intent, final-gate, seal, permit, and authority chain
while keeping `authority_granted=false`, `contracts_ready=false`, `write_allowed=false`,
`admission_allowed=false`, `live_admission_enabled=false`, `body_mutation_allowed=false`, `body_target=none`, and
`mutates_state=false`.
`make admission-weighted-admission-resonance-graft-admission-final-gate-observation-boundary-consumer` proves
