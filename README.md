...
make admission-qloop-sweep # compare qloop strict, question hints, QA-framed answers, and statement fallback
...
`make body-smoke` is the shared-body contract check: it builds Janus, Resonance, nano, chorus, DOE, KK, metabolism, runs Go tests, checks shadow dream admission receipts with replay guards and admission-policy verdicts, runs the shadow receipt sampler, and only probes runtime from a temporary state directory. `make admission-shadow-sample-broad` runs the tracked broad corpus when tuning the admission policy beyond the built-in smoke probes; `make admission-route-compare` compares direct nano, chorus, and qloop candidates through the same shadow gate before any live widening, resolving the default nano GGUF from the main shared checkout when invoked inside a git worktree. `make admission-qloop-sweep` compares strict qloop routing, question-source hinting, QA-framed qloop answer context, loose question-source hinting, and statement fallback, then records qloop route-picker stats (`picker_seen`, `qsrc`, `ssrc`, `routes`, `score_drop`), gate-reason telemetry, per-sample receipts, and surface-debt telemetry before any qloop default changes; qloop admission selects one candidate line instead of joining several qloop lines into a false sentence, and a live route is not a production winner until its speech clears the quality gate.
