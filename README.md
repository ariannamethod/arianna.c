...
make admission-qloop-sweep # compare qloop strict, question-source frames, QA-framed answers, and statement fallback
...
`make body-smoke` is the shared-body contract check: it builds Janus, Resonance, nano, chorus, DOE, KK, metabolism, runs Go tests, checks shadow dream admission receipts with replay guards and admission-policy verdicts, runs the shadow receipt sampler, and only probes runtime from a temporary state directory. `make admission-shadow-sample-broad` runs the tracked broad corpus when tuning the admission policy beyond the built-in smoke probes; `make admission-route-compare` compares direct nano, chorus, and qloop candidates through the same shadow gate before any live widening, resolving the default nano GGUF from the main shared checkout when invoked inside a git worktree. `make admission-qloop-sweep` compares strict qloop routing, legacy question-source hinting, train-shaped `Q:/A:` and `User:/Arianna:` question-source frames, QA-framed qloop answer context, loose question-source hinting, and statement fallback, then records qloop route-picker stats (`picker_seen`, `qsrc`, `ssrc`, `routes`, `score_drop`), gate-reason telemetry, per-sample receipts, a per-seed `sample_coverage` matrix with `best_semantic_*` and `least_debt_*` candidates, surface-debt telemetry, and prompt-class semantic scores before any qloop default changes; qloop admission selects one candidate line instead of joining several qloop lines into a false sentence, and when the admission harness knows the prompt class it uses semantic score as a tie-break after surface safety. A live route is not a production winner until its speech clears the quality gate. `make admission-qloop-sweep-broad` runs the same matrix with `A2A_QLOOP_SWEEP_LIMIT=6` by default for wider manual tuning without slowing the normal smoke.
