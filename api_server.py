...
async def health():
    ...
    if not tokenizer:
        raise HTTPException(
            status_code=503,
            detail="Tokenizer not found. Ensure weights/tokenizer.json exists."
        )

    return {"status": "healthy", "binary": binary, "weights": weights, "tokenizer": tokenizer}
...
async def generate(request: GenerateRequest):
    ...
    if not tokenizer:
        return GenerateResponse(
            ...
            mode=request.mode,
            success=False,
            error="Tokenizer not found. Ensure weights/tokenizer.json exists."
        )
