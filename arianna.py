...
def run_arianna_batch(commands, weights="weights/arianna.bin", tokenizer="weights/tokenizer.json", max_tokens=50, temp=0.8):
    """Run arianna with a batch of commands, return output."""
    binary = "./bin/arianna_dynamic"

    if not os.path.exists(binary):
        raise FileNotFoundError(f"No arianna binary found. Run 'make dynamic' first.")
    ...
    cmd = [binary, weights, tokenizer, "--repl", str(max_tokens), str(temp)]
...
class AriannaSession:
    ...
    def __init__(self, weights="weights/arianna.bin", tokenizer="weights/tokenizer.json", max_tokens=100, temp=0.8):
...
def train_mathbrain(weights="weights/arianna.bin", tokenizer="weights/tokenizer.json", n_problems=100, batch_size=10):
    ...
    for phase_problems, min_n, max_n in phases:
        ...
        # Run batch
        output = run_arianna_batch(commands, weights=weights, tokenizer=tokenizer, max_tokens=10)
...
def interactive_mode(weights="weights/arianna.bin", tokenizer="weights/tokenizer.json", max_tokens=100, temp=0.8):
    ...
    while True:
        try:
            ...
            # Run single command
            output = run_arianna_batch([user_input], weights=weights, tokenizer=tokenizer, max_tokens=max_tokens, temp=temp)
...
def main():
    ...
    parser.add_argument("--tokenizer", default="weights/tokenizer.json", help="Path to tokenizer")
    ...
    try:
        if args.train_math:
            train_mathbrain(args.weights, args.tokenizer, args.train_math)
        else:
            interactive_mode(args.weights, args.tokenizer, args.tokens, args.temp)
