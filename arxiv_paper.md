...
## Why 10 Languages + DSL?

Arianna is implemented across 10 programming languages and a custom DSL: C, Go, Python, Zig, Julia, Lua, JavaScript, HTML, Shell, Makefile, and the Arianna DSL. CUDA is used only during training on Lambda (H100 GPUs), not in the organism itself. This is not accidental polyglotism. Each language handles what it does best:
...
Arianna runs on consumer hardware—MacBook Pro 2019 (i5 1.4GHz, 8GB RAM, Intel Iris Plus Graphics)—demonstrating that organism-level intelligence does not require datacenter-scale compute. She is implemented across 10 programming languages, each chosen for its strengths, forming an ecological whole.
