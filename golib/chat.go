...
func chatLiveRouteTurnCandidateAdmissionResonanceGraftPreflightLine(preflight admissionLiveRouteTurnCandidateAdmissionResonanceGraftPreflight) string {
	return fmt.Sprintf("│  · live-route candidate admission resonance graft preflight dry-run: class=%s route=%s source=%s boundary=%s observation=%s receiver=%s intent=%s final_gate=%s seal=%s permit=%s readiness=%s ledger_verification=%s preflight_kind=%s preflight_mode=%s preflight_stage=%s causal_id=%s preflight_hash=%s read_back_hash=%s source_boundary_causal_id=%s source_boundary_read_back_hash=%s admission_required=%t shadow_only=%t graft_allowed=%t raw_text_allowed=%t janus_surface_allowed=%t cooc_learning_allowed=%t delta_harvest_allowed=%t body_mutation_allowed=%t rollback_required=%t preflight_state=%s preflight_action=%s preflight_target=%s preflight_target_kind=%s preflight_target_mode=%s receipt_shape=%s dry_run_only=%t boundary_verified=%t observation_verified=%t receiver_verified=%t intent_verified=%t final_gate_verified=%t seal_verified=%t permit_verified=%t readiness_verified=%t ledger_verified=%t writer_ready=%t rollback_ready=%t ledger_ready=%t preflight_ready=%t contracts_ready=%t write_allowed=%t admission_allowed=%t live_ready=%t live_enabled=%t mutates=%t admission_resonance_graft_preflight_id=%s passed=%t%s",
		preflight.PromptClass, preflight.Route, preflight.Source,
		preflight.AdmissionResonanceGraftBoundaryID, preflight.AdmissionResonanceObservationID,
		preflight.AdmissionResonanceReceiverID, preflight.AdmissionResonanceIntentID,
		preflight.AdmissionFinalGateID, preflight.AdmissionSealID, preflight.AdmissionPermitID,
		preflight.AdmissionReadinessID, preflight.LedgerVerificationID,
		preflight.AdmissionResonanceGraftPreflightKind,
		preflight.AdmissionResonanceGraftPreflightMode,
		preflight.AdmissionResonanceGraftPreflightStage,
		...
		preflight.SourceAdmissionResonanceGraftBoundaryCausalID,
		preflight.SourceAdmissionResonanceGraftBoundaryReadBackHash,
		preflight.AdmissionResonanceGraftPreflightAdmissionRequired,
		...
		preflight.AdmissionResonanceGraftPreflightState,
		preflight.AdmissionResonanceGraftPreflightAction,
		preflight.AdmissionResonanceGraftPreflightTarget,
		preflight.AdmissionResonanceGraftPreflightTargetKind,
		preflight.AdmissionResonanceGraftPreflightTargetMode,
		preflight.AdmissionResonanceGraftPreflightReceiptShape,
		preflight.AdmissionResonanceGraftPreflightDryRunOnly,
		...
		preflight.ContractsReady, preflight.WriteAllowed, preflight.AdmissionAllowed,
		preflight.LiveReady, preflight.LiveAdmissionEnabled, preflight.MutatesState,
		preflight.AdmissionResonanceGraftPreflightID, preflight.Passed,
		chatLiveRouteReasonSuffix(preflight.Reason))
}
...
func chatLiveRouteTurnCandidateAdmissionResonanceGraftCandidateLine(candidate admissionLiveRouteTurnCandidateAdmissionResonanceGraftCandidate) string {
	return fmt.Sprintf("│  · live-route candidate admission resonance graft candidate dry-run: class=%s route=%s source=%s gate=%s preflight=%s boundary=%s observation=%s receiver=%s intent=%s final_gate=%s seal=%s permit=%s readiness=%s ledger_verification=%s candidate_kind=%s candidate_mode=%s candidate_stage=%s causal_id=%s candidate_hash=%s read_back_hash=%s source_gate_causal_id=%s source_gate_read_back_hash=%s admission_required=%t shadow_only=%t graft_allowed=%t raw_text_allowed=%t janus_surface_allowed=%t cooc_learning_allowed=%t delta_harvest_allowed=%t body_mutation_allowed=%t rollback_required=%t candidate_state=%s candidate_action=%s candidate_target=%s candidate_target_kind=%s candidate_target_mode=%s receipt_shape=%s dry_run_only=%t gate_verified=%t preflight_verified=%t boundary_verified=%t observation_verified=%t receiver_verified=%t intent_verified=%t final_gate_verified=%t seal_verified=%t permit_verified=%t readiness_verified=%t ledger_verified=%t writer_ready=%t rollback_ready=%t ledger_ready=%t candidate_ready=%t contracts_ready=%t write_allowed=%t admission_allowed=%t live_ready=%t live_enabled=%t mutates=%t admission_resonance_graft_candidate_id=%s passed=%t%s",
		candidate.PromptClass, candidate.Route, candidate.Source,
		candidate.AdmissionResonanceGraftGateID,
		candidate.AdmissionResonanceGraftPreflightID,
		candidate.AdmissionResonanceGraftBoundaryID, candidate.AdmissionResonanceObservationID,
		candidate.AdmissionResonanceReceiverID, candidate.AdmissionResonanceIntentID,
		candidate.AdmissionFinalGateID, candidate.AdmissionSealID, candidate.AdmissionPermitID,
		candidate.AdmissionReadinessID, candidate.LedgerVerificationID,
		candidate.AdmissionResonanceGraftCandidateKind,
		candidate.AdmissionResonanceGraftCandidateMode,
		candidate.AdmissionResonanceGraftCandidateStage,
		...
		candidate.SourceAdmissionResonanceGraftGateCausalID,
		candidate.SourceAdmissionResonanceGraftGateReadBackHash,
		candidate.AdmissionResonanceGraftCandidateAdmissionRequired,
		...
		candidate.AdmissionResonanceGraftCandidateState,
		candidate.AdmissionResonanceGraftCandidateAction,
		candidate.AdmissionResonanceGraftCandidateTarget,
		candidate.AdmissionResonanceGraftCandidateTargetKind,
		candidate.AdmissionResonanceGraftCandidateTargetMode,
		candidate.AdmissionResonanceGraftCandidateReceiptShape,
		candidate.AdmissionResonanceGraftCandidateDryRunOnly,
		...
		candidate.ContractsReady, candidate.WriteAllowed, candidate.AdmissionAllowed,
		candidate.LiveReady, candidate.LiveAdmissionEnabled, candidate.MutatesState,
		candidate.AdmissionResonanceGraftCandidateID, candidate.Passed,
		chatLiveRouteReasonSuffix(candidate.Reason))
}

func chatLiveRouteTurnCandidateAdmissionResonanceGraftCandidateStoreLine(store admissionLiveRouteTurnCandidateAdmissionResonanceGraftCandidateStore) string {
	return fmt.Sprintf("│  · live-route candidate admission resonance graft candidate store dry-run: class=%s route=%s source=%s candidate=%s gate=%s preflight=%s boundary=%s observation=%s receiver=%s intent=%s final_gate=%s seal=%s permit=%s readiness=%s ledger_verification=%s store_kind=%s store_mode=%s store_stage=%s causal_id=%s store_hash=%s read_back_hash=%s source_candidate_causal_id=%s source_candidate_read_back_hash=%s admission_required=%t shadow_only=%t graft_allowed=%t raw_text_allowed=%t janus_surface_allowed=%t cooc_learning_allowed=%t delta_harvest_allowed=%t body_mutation_allowed=%t rollback_required=%t append_only=%t read_back=%t receipt_persisted=%t receipt_verified=%t store_state=%s store_action=%s store_target=%s store_target_kind=%s store_target_mode=%s receipt_shape=%s dry_run_only=%t candidate_verified=%t gate_verified=%t preflight_verified=%t boundary_verified=%t observation_verified=%t receiver_verified=%t intent_verified=%t final_gate_verified=%t seal_verified=%t permit_verified=%t readiness_verified=%t ledger_verified=%t writer_ready=%t rollback_ready=%t ledger_ready=%t store_ready=%t contracts_ready=%t write_allowed=%t admission_allowed=%t live_ready=%t live_enabled=%t mutates=%t admission_resonance_graft_candidate_store_id=%s passed=%t%s",
		store.PromptClass, store.Route, store.Source,
		store.AdmissionResonanceGraftCandidateID,
		store.AdmissionResonanceGraftGateID,
		store.AdmissionResonanceGraftPreflightID,
		store.AdmissionResonanceGraftBoundaryID, store.AdmissionResonanceObservationID,
		store.AdmissionResonanceReceiverID, store.AdmissionResonanceIntentID,
		store.AdmissionFinalGateID, store.AdmissionSealID, store.AdmissionPermitID,
		store.AdmissionReadinessID, store.LedgerVerificationID,
		store.AdmissionResonanceGraftCandidateStoreKind,
		store.AdmissionResonanceGraftCandidateStoreMode,
		store.AdmissionResonanceGraftCandidateStoreStage,
		...
		store.SourceAdmissionResonanceGraftCandidateCausalID,
		store.SourceAdmissionResonanceGraftCandidateReadBackHash,
		store.AdmissionResonanceGraftCandidateStoreAdmissionRequired,
		...
		store.AdmissionResonanceGraftCandidateStoreState,
		store.AdmissionResonanceGraftCandidateStoreAction,
		store.AdmissionResonanceGraftCandidateStoreTarget,
		store.AdmissionResonanceGraftCandidateStoreTargetKind,
		store.AdmissionResonanceGraftCandidateStoreTargetMode,
		store.AdmissionResonanceGraftCandidateStoreReceiptShape,
		store.AdmissionResonanceGraftCandidateStoreDryRunOnly,
		...
		store.ContractsReady, store.WriteAllowed, store.AdmissionAllowed,
		store.LiveReady, store.LiveAdmissionEnabled, store.MutatesState,
		store.AdmissionResonanceGraftCandidateStoreID, store.Passed,
...
func chatLiveRouteTurnCandidateAdmissionResonanceGraftAdmissionProofLine(proof admissionLiveRouteTurnCandidateAdmissionResonanceGraftAdmissionProof) string {
	return fmt.Sprintf("│  · live-route candidate admission resonance graft admission proof dry-run: class=%s route=%s source=%s reader=%s store=%s candidate=%s gate=%s observation=%s final_gate=%s ledger_verification=%s proof_kind=%s proof_mode=%s proof_stage=%s causal_id=%s proof_hash=%s replay_hash=%s read_back_hash=%s source_reader_causal_id=%s source_reader_read_back_hash=%s admission_required=%t shadow_only=%t graft_allowed=%t raw_text_allowed=%t janus_surface_allowed=%t cooc_learning_allowed=%t delta_harvest_allowed=%t body_mutation_allowed=%t rollback_required=%t proof_state=%s proof_action=%s proof_target=%s proof_target_kind=%s proof_target_mode=%s receipt_shape=%s dry_run_only=%t reader_verified=%t store_verified=%t candidate_verified=%t ledger_verified=%t replay_verified=%t hash_verified=%t proof_read_back_verified=%t proof_ready=%t contracts_ready=%t write_allowed=%t admission_allowed=%t live_ready=%t live_enabled=%t mutates=%t admission_resonance_graft_admission_proof_id=%s passed=%t%s",
		proof.PromptClass, proof.Route, proof.Source,
		proof.AdmissionResonanceGraftCandidateStoreReaderID,
		proof.AdmissionResonanceGraftCandidateStoreID,
		proof.AdmissionResonanceGraftCandidateID,
		proof.AdmissionResonanceGraftGateID,
		proof.AdmissionResonanceObservationID,
		proof.AdmissionFinalGateID,
		proof.LedgerVerificationID,
		proof.AdmissionResonanceGraftAdmissionProofKind,
		proof.AdmissionResonanceGraftAdmissionProofMode,
		proof.AdmissionResonanceGraftAdmissionProofStage,
		proof.AdmissionResonanceGraftAdmissionProofCausalID,
		proof.AdmissionResonanceGraftAdmissionProofHash,
		proof.AdmissionResonanceGraftAdmissionProofReplayHash,
		proof.AdmissionResonanceGraftAdmissionProofReadBackHash,
		proof.SourceAdmissionResonanceGraftCandidateStoreReaderCausalID,
		proof.SourceAdmissionResonanceGraftCandidateStoreReaderReadBackHash,
		proof.AdmissionResonanceGraftAdmissionProofAdmissionRequired,
		proof.AdmissionResonanceGraftAdmissionProofShadowOnly,
		proof.AdmissionResonanceGraftAdmissionProofGraftAllowed,
		proof.AdmissionResonanceGraftAdmissionProofRawDreamTextAllowed,
		proof.AdmissionResonanceGraftAdmissionProofJanusSurfaceAllowed,
		proof.AdmissionResonanceGraftAdmissionProofCoocLearningAllowed,
		proof.AdmissionResonanceGraftAdmissionProofDeltaHarvestAllowed,
		proof.AdmissionResonanceGraftAdmissionProofBodyMutationAllowed,
		proof.AdmissionResonanceGraftAdmissionProofRollbackRequired,
		proof.AdmissionResonanceGraftAdmissionProofState,
		proof.AdmissionResonanceGraftAdmissionProofAction,
		proof.AdmissionResonanceGraftAdmissionProofTarget,
		proof.AdmissionResonanceGraftAdmissionProofTargetKind,
		proof.AdmissionResonanceGraftAdmissionProofTargetMode,
		proof.AdmissionResonanceGraftAdmissionProofReceiptShape,
		proof.AdmissionResonanceGraftAdmissionProofDryRunOnly,
		proof.AdmissionResonanceGraftAdmissionProofReaderVerified,
		proof.AdmissionResonanceGraftAdmissionProofStoreVerified,
		proof.AdmissionResonanceGraftAdmissionProofCandidateVerified,
		proof.AdmissionResonanceGraftAdmissionProofLedgerVerified,
		proof.AdmissionResonanceGraftAdmissionProofReplayVerified,
		proof.AdmissionResonanceGraftAdmissionProofHashVerified,
		proof.AdmissionResonanceGraftAdmissionProofReadBackVerified,
		proof.AdmissionResonanceGraftAdmissionProofReady,
		proof.ContractsReady, proof.WriteAllowed, proof.AdmissionAllowed,
		proof.LiveReady, proof.LiveAdmissionEnabled, proof.MutatesState,
		proof.AdmissionResonanceGraftAdmissionProofID, proof.Passed,
		chatLiveRouteReasonSuffix(proof.Reason))
}
...
func chatLiveRouteTurnCandidateChainDryRunLines(obs admissionLiveRouteTurnObservation) []string {
	...
	resonanceGraftAdmissionProofDryRun := admissionLiveRouteTurnCandidateAdmissionResonanceGraftAdmissionProofDryRun()
	resonanceGraftCandidateStoreReaderNeeded := resonanceGraftCandidateStoreReaderDryRun || resonanceGraftAdmissionProofDryRun
	resonanceGraftCandidateStoreNeeded := resonanceGraftCandidateStoreDryRun || resonanceGraftCandidateStoreReaderNeeded
					...
					if resonanceIntentNeeded {
						...
						if resonanceReceiverNeeded {
							...
							if resonanceObservationNeeded {
								...
								if resonanceGraftBoundaryNeeded {
									...
									if resonanceGraftPreflightNeeded {
										...
										if resonanceGraftGateNeeded {
											...
											if resonanceGraftCandidateNeeded {
												candidate := admissionLiveRouteTurnCandidateAdmissionResonanceGraftCandidateForGate(gate)
												if resonanceGraftCandidateDryRun {
													if err := recordAdmissionLiveRouteTurnCandidateAdmissionResonanceGraftCandidate(candidate); err != nil {
														return append(lines, fmt.Sprintf("│  · live-route candidate admission resonance graft candidate dry-run log failed: %v", err))
													}
												...
												if resonanceGraftCandidateStoreNeeded {
													...
													if resonanceGraftCandidateStoreReaderNeeded {
														...
														if resonanceGraftAdmissionProofDryRun {
															proof := admissionLiveRouteTurnCandidateAdmissionResonanceGraftAdmissionProofForStoreReader(reader)
															if err := recordAdmissionLiveRouteTurnCandidateAdmissionResonanceGraftAdmissionProof(proof); err != nil {
																return append(lines, fmt.Sprintf("│  · live-route candidate admission resonance graft admission proof dry-run log failed: %v", err))
															}
															lines = append(lines, chatLiveRouteTurnCandidateAdmissionResonanceGraftAdmissionProofLine(proof))
														}
													}
												}
											}
