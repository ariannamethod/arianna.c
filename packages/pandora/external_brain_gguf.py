...
# Supports both 84-token (20M) and 86-token (34M) models
ARIANNA_VOCAB = None
            ...
            try:
                with open(tok_path) as f:
                    ...
                    if 'char_to_id' in data:
                        ...
                        print(f"[external_brain_gguf] Loaded vocab ({len(ARIANNA_VOCAB)} tokens) from {tok_path.name}", file=sys.stderr)
                        return ARIANNA_VOCAB
            except Exception as e:
                print(f"[external_brain_gguf] Warning: failed to load {tok_path}: {e}", file=sys.stderr)

    # Fallback to hardcoded 84-token vocab (legacy)
    print("[external_brain_gguf] Warning: using fallback vocab (84 tokens)", file=sys.stderr)
    ARIANNA_VOCAB = {
        "\n": 0, " ": 1, "\"": 2, "%": 3, "'": 4, "(": 5, ")": 6, "*": 7,
