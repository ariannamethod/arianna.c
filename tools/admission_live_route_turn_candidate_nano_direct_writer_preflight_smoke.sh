#!/usr/bin/env bash
# admission_live_route_turn_candidate_nano_direct_writer_preflight_smoke.sh - real nano direct -> writer preflight dry-run.
#
# Runs the nano-direct chat shadow chain through decision, promotion, switch,
# enable-gate, live-stage, and writer-preflight receipts. The preflight may
# only prove that writer/rollback contracts are absent; no live admission or
...
WORKDIR="${A2A_ADMISSION_LIVE_ROUTE_TURN_CANDIDATE_NANO_DIRECT_WRITER_PREFLIGHT_WORKDIR:-$(mktemp -d "${tmp_root%/}/arianna-live-route-nano-direct-writer-preflight.XXXXXX")}"
...
die() {
    echo "[admission-live-route-turn-candidate-nano-direct-writer-preflight-smoke] FAIL: $*" >&2
    if [[ -f "$RUN_LOG" ]]; then
        tail -n 360 "$RUN_LOG" >&2 || true
    fi
    exit 1
}
...
if ! A2A_ADMISSION_LIVE_ROUTE_TURN_CANDIDATE_NANO_DIRECT_CHAT_SHADOW_WORKDIR="$WORKDIR" \
    ...
    die "nano-direct chat shadow smoke with writer preflight failed"
fi
...
grep -q '"writer_action":"require_writer_contract"' "$WRITER_PREFLIGHT_LOG" || die "writer action missing"
grep -q '"rollback_state":"absent"' "$WRITER_PREFLIGHT_LOG" || die "rollback should be absent"
grep -q '"rollback_action":"require_rollback_contract"' "$WRITER_PREFLIGHT_LOG" || die "rollback action missing"
grep -q '"admission_live_stage_id":"stage-' "$WRITER_PREFLIGHT_LOG" || die "writer preflight live stage id missing"
grep -q '"source_stage_passed":true' "$WRITER_PREFLIGHT_LOG" || die "writer preflight did not consume a passed live stage"
grep -q '"live_ready":true' "$WRITER_PREFLIGHT_LOG" || die "writer preflight live-ready verdict missing"
grep -q '"live_admission_enabled":false' "$WRITER_PREFLIGHT_LOG" || die "writer preflight should not enable live admission"
grep -q '"admission_allowed":false' "$WRITER_PREFLIGHT_LOG" || die "writer preflight should not allow admission"
grep -q '"manual_enable_requested":true' "$WRITER_PREFLIGHT_LOG" || die "writer preflight should preserve manual enable"
grep -q '"enable_key_matched":true' "$WRITER_PREFLIGHT_LOG" || die "writer preflight should preserve key match"
grep -q '"requires_writer":true' "$WRITER_PREFLIGHT_LOG" || die "writer preflight should require writer"
grep -q '"writer_ready":false' "$WRITER_PREFLIGHT_LOG" || die "writer preflight must keep writer absent"
grep -q '"requires_rollback":true' "$WRITER_PREFLIGHT_LOG" || die "writer preflight should require rollback"
grep -q '"rollback_ready":false' "$WRITER_PREFLIGHT_LOG" || die "writer preflight must keep rollback absent"
...
grep -q 'live-route candidate admission writer preflight dry-run: class=dream route=direct source=direct stage=staged_dry_run stage_id=stage-' "$RUN_LOG" || die "writer preflight chat line missing"
grep -q 'writer=absent writer_action=require_writer_contract rollback=absent rollback_action=require_rollback_contract writer_preflight_id=writer-' "$RUN_LOG" || die "writer preflight id line missing"
grep -q 'write_allowed=false admission_allowed=false live_ready=true live_enabled=false mutates=false passed=true reason=writer and rollback absent; live admission remains staged only' "$RUN_LOG" || die "writer preflight dry-run verdict missing"

echo "[admission-live-route-turn-candidate-nano-direct-writer-preflight-smoke] pass: decision=$DECISION_LOG promotion=$PROMOTION_LOG switch=$SWITCH_LOG enable_gate=$ENABLE_GATE_LOG live_stage=$LIVE_STAGE_LOG writer_preflight=$WRITER_PREFLIGHT_LOG"
