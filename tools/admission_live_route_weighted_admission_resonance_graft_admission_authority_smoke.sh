#!/usr/bin/env bash
# admission_live_route_weighted_admission_resonance_graft_admission_authority_smoke.sh - block weighted Resonance graft admission authority behind blocked permit.
...
WORKDIR="${A2A_ADMISSION_LIVE_ROUTE_WEIGHTED_ADMISSION_RESONANCE_GRAFT_ADMISSION_AUTHORITY_WORKDIR:-$(mktemp -d "${tmp_root%/}/arianna-live-route-weighted-admission-resonance-graft-admission-authority.XXXXXX")}"
...
GRAFT_ADMISSION_AUTHORITY_REPORT="${A2A_ADMISSION_LIVE_ROUTE_WEIGHTED_ADMISSION_RESONANCE_GRAFT_ADMISSION_AUTHORITY_REPORT:-$WORKDIR/live_route_weighted_admission_resonance_graft_admission_authority.json}"
...
die() {
    echo "[admission-live-route-weighted-admission-resonance-graft-admission-authority-smoke] FAIL: $*" >&2
...
if ! A2A_ADMISSION_LIVE_ROUTE_WEIGHTED_ADMISSION_RESONANCE_GRAFT_ADMISSION_PERMIT_WORKDIR="$PERMIT_WORKDIR" \
    A2A_ADMISSION_LIVE_ROUTE_WEIGHTED_ADMISSION_RESONANCE_GRAFT_ADMISSION_PERMIT_REPORT="$GRAFT_ADMISSION_PERMIT_REPORT" \
    bash "$ROOT/tools/admission_live_route_weighted_admission_resonance_graft_admission_permit_smoke.sh" >"$PERMIT_LOG" 2>&1; then
...
if ! bash "$ROOT/tools/admission_live_route_weighted_admission_resonance_graft_admission_authority.sh" "$GRAFT_ADMISSION_PERMIT_REPORT" "$GRAFT_ADMISSION_AUTHORITY_REPORT" >"$AUTHORITY_LOG" 2>&1; then
    die "weighted admission resonance graft admission authority rejected permit report"
fi
...
require_grep '"schema": "arianna.live_route_weighted_admission_resonance_graft_admission_authority.v1"' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "authority schema"
require_grep '"status": "shadow_graft_admission_authority_blocked_dry_run"' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "authority status"
require_grep '"target": "live_route_admission_next_step"' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "authority target"
require_grep '"target_kind": "weighted_internal_world_shadow_graft_admission_authority"' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "authority target kind"
require_grep '"target_mode": "closed_authority_guard_dry_run"' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "authority target mode"
require_grep '"action": "block_weighted_resonance_shadow_graft_admission_permit_blocked_dry_run"' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "authority action"
...
require_grep '"admission_authority_target_kind": "weighted_internal_world_shadow_graft_admission_permit"' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "admission authority target kind"
...
require_grep '"weighted_admission_resonance_graft_admission_authority_ready": true' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "weighted authority ready"
require_grep '"weighted_admission_resonance_graft_admission_permit_consumed": true' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "permit consumed"
require_grep '"weighted_admission_resonance_graft_admission_permit_required": true' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "permit required"
require_grep '"next_step_blocked_without_resonance_graft_admission_authority": true' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "next-step block"
require_grep '"weighted_admission_resonance_graft_admission_authority_id": "weighted-resonance-graft-admission-authority-id-' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "authority id"
require_grep '"causal_id": "weighted-resonance-graft-admission-authority-causal-' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "authority causal id"
require_grep '"admission_authority_hash": "weighted-resonance-graft-admission-authority-' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "authority hash"
require_grep '"admission_authority_read_back_hash": "weighted-resonance-graft-admission-authority-read-' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "authority read-back hash"
require_grep '"source_schema": "arianna.live_route_weighted_admission_resonance_graft_admission_permit.v1"' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "source permit schema"
require_grep '"source_status": "shadow_graft_admission_permit_blocked_dry_run"' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "source permit status"
require_grep '"source_admission_readiness_schema": "arianna.live_route_weighted_admission_resonance_graft_admission_readiness.v1"' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "source readiness schema"
require_grep '"source_weighted_admission_resonance_graft_admission_permit_id": "weighted-resonance-graft-admission-permit-id-' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "source permit id"
require_grep '"source_weighted_admission_resonance_graft_admission_permit_ready": true' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "source permit ready"
require_grep '"source_weighted_admission_resonance_graft_admission_permit_causal_id": "weighted-resonance-graft-admission-permit-causal-' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "source permit causal id"
require_grep '"source_weighted_admission_resonance_graft_admission_permit_hash": "weighted-resonance-graft-admission-permit-' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "source permit hash"
require_grep '"source_weighted_admission_resonance_graft_admission_permit_read_back_hash": "weighted-resonance-graft-admission-permit-read-' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "source permit read-back"
require_grep '"source_admission_permit_report_receipt_shape": "weighted_resonance_shadow_graft_admission_permit_receipt"' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "source permit report receipt shape"
...
require_grep '"source_admission_permit_target_kind": "weighted_internal_world_shadow_graft_admission_readiness"' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "source permit target kind"
...
require_grep '"source_admission_permit_reason": "weighted resonance shadow graft admission permit blocked by blocked readiness; manual permit remains closed"' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "source permit reason"
...
require_grep '"reason": "weighted resonance shadow graft admission authority blocked by blocked permit; live authority remains closed"' "$GRAFT_ADMISSION_AUTHORITY_REPORT" "authority reason"
require_grep '\[admission-live-route-weighted-admission-resonance-graft-admission-authority\] pass:' "$AUTHORITY_LOG" "authority pass line"

echo "[admission-live-route-weighted-admission-resonance-graft-admission-authority-smoke] pass: resonance_graft_admission_permit_report=$GRAFT_ADMISSION_PERMIT_REPORT resonance_graft_admission_authority_report=$GRAFT_ADMISSION_AUTHORITY_REPORT"
