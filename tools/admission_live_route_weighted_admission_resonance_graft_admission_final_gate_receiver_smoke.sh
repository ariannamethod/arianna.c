#!/usr/bin/env bash
# admission_live_route_weighted_admission_resonance_graft_admission_final_gate_receiver_smoke.sh - preview final-gate receiver from compact weighted graft admission final-gate intent.
...
WORKDIR="${A2A_ADMISSION_LIVE_ROUTE_WEIGHTED_ADMISSION_RESONANCE_GRAFT_ADMISSION_FINAL_GATE_RECEIVER_WORKDIR:-$(mktemp -d "${tmp_root%/}/arianna-live-route-weighted-admission-resonance-graft-admission-final-gate-receiver.XXXXXX")}"
INTENT_WORKDIR="$WORKDIR/final_gate_intent"
...
GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT="${A2A_ADMISSION_LIVE_ROUTE_WEIGHTED_ADMISSION_RESONANCE_GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT:-$WORKDIR/live_route_weighted_admission_resonance_graft_admission_final_gate_receiver.json}"
...
die() {
    echo "[admission-live-route-weighted-admission-resonance-graft-admission-final-gate-receiver-smoke] FAIL: $*" >&2
...
if ! A2A_ADMISSION_LIVE_ROUTE_WEIGHTED_ADMISSION_RESONANCE_GRAFT_ADMISSION_FINAL_GATE_INTENT_WORKDIR="$INTENT_WORKDIR" \
    ...
    bash "$ROOT/tools/admission_live_route_weighted_admission_resonance_graft_admission_final_gate_intent_smoke.sh" >"$INTENT_LOG" 2>&1; then
...
if ! bash "$ROOT/tools/admission_live_route_weighted_admission_resonance_graft_admission_final_gate_receiver.sh" "$GRAFT_ADMISSION_FINAL_GATE_INTENT_REPORT" "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" >"$RECEIVER_LOG" 2>&1; then
    die "weighted admission resonance graft admission final gate receiver rejected intent report"
fi
...
require_grep '"schema": "arianna.live_route_weighted_admission_resonance_graft_admission_final_gate_receiver.v1"' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "receiver schema"
require_grep '"status": "shadow_graft_admission_final_gate_receiver_previewed_dry_run"' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "receiver status"
require_grep '"target": "live_route_admission_next_step"' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "receiver target"
require_grep '"target_kind": "weighted_internal_world_shadow_graft_admission_final_gate_receiver"' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "receiver target kind"
...
require_grep '"action": "preview_weighted_resonance_shadow_graft_admission_final_gate_receiver_dry_run"' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "receiver action"
...
require_grep '"receipt_shape": "weighted_resonance_shadow_graft_admission_final_gate_receiver_receipt"' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "receipt shape"
...
require_grep '"admission_final_gate_receiver_target_kind": "weighted_internal_world_shadow_graft_admission_final_gate_intent"' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "receiver target kind field"
...
require_grep '"weighted_admission_resonance_graft_admission_final_gate_receiver_ready": true' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "weighted receiver ready"
require_grep '"weighted_admission_resonance_graft_admission_final_gate_intent_consumed": true' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "intent consumed"
require_grep '"weighted_admission_resonance_graft_admission_final_gate_intent_required": true' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "intent required"
require_grep '"next_step_blocked_without_resonance_graft_admission_final_gate_receiver": true' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "next-step block"
require_grep '"weighted_admission_resonance_graft_admission_final_gate_receiver_id": "weighted-resonance-graft-admission-final-gate-receiver-id-' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "receiver id"
require_grep '"causal_id": "weighted-resonance-graft-admission-final-gate-receiver-causal-' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "receiver causal id"
require_grep '"admission_final_gate_receiver_pre_state_hash": "weighted-resonance-graft-admission-final-gate-receiver-pre-' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "receiver pre hash"
require_grep '"admission_final_gate_receiver_post_state_hash": "weighted-resonance-graft-admission-final-gate-receiver-post-' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "receiver post hash"
require_grep '"admission_final_gate_receiver_state_delta_hash": "weighted-resonance-graft-admission-final-gate-receiver-delta-' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "receiver delta hash"
require_grep '"source_schema": "arianna.live_route_weighted_admission_resonance_graft_admission_final_gate_intent.v1"' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "source intent schema"
require_grep '"source_status": "shadow_graft_admission_final_gate_intent_blocked_dry_run"' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "source intent status"
...
require_grep '"source_weighted_admission_resonance_graft_admission_final_gate_intent_id": "weighted-resonance-graft-admission-final-gate-intent-id-' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "source intent id"
require_grep '"source_weighted_admission_resonance_graft_admission_final_gate_intent_ready": true' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "source weighted intent ready"
require_grep '"source_weighted_admission_resonance_graft_admission_final_gate_intent_causal_id": "weighted-resonance-graft-admission-final-gate-intent-causal-' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "source intent causal"
require_grep '"source_admission_final_gate_intent_hash": "weighted-resonance-graft-admission-final-gate-intent-' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "source intent hash"
require_grep '"source_admission_final_gate_intent_read_back_hash": "weighted-resonance-graft-admission-final-gate-intent-read-' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "source intent read-back"
require_grep '"source_admission_final_gate_intent_receipt_shape": "weighted_resonance_shadow_graft_admission_final_gate_intent_receipt"' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "source intent receipt shape"
...
require_grep '"source_admission_final_gate_intent_reason": "weighted resonance shadow graft admission final gate intent drafted from blocked final gate; live admission remains closed"' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "source reason"
...
require_grep '"reason": "weighted resonance shadow graft admission final gate receiver previewed from blocked final gate intent; live admission remains closed"' "$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT" "receiver reason"
require_grep '\[admission-live-route-weighted-admission-resonance-graft-admission-final-gate-receiver\] pass:' "$RECEIVER_LOG" "receiver pass line"

echo "[admission-live-route-weighted-admission-resonance-graft-admission-final-gate-receiver-smoke] pass: resonance_graft_admission_final_gate_intent_report=$GRAFT_ADMISSION_FINAL_GATE_INTENT_REPORT resonance_graft_admission_final_gate_receiver_report=$GRAFT_ADMISSION_FINAL_GATE_RECEIVER_REPORT"
