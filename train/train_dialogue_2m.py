#!/usr/bin/env python3
"""
Train Dialogue Model for Arianna - 2.2M params

Full model training (not LoRA) on dialogue corpus.
Architecture: dim=256, layers=4, hidden=512 = ~2.2M params

Usage:
    python train/train_dialogue_2m.py

Output:
    weights/arianna_dialogue_2m.bin - full model weights
"""
...
# ============================================================
# Configuration - 2.2M params
# ============================================================

DIM = 256           # Increased from 128
N_LAYERS = 4
N_HEADS = 8         # More heads for larger dim
...
class AriannaDialogue2M(nn.Module):
    def __init__(self):
        super().__init__()
...
def train():
    print("=" * 60)
    print("Training Arianna Dialogue 2.2M")
    print("=" * 60)
    ...
    # Create model
    model = AriannaDialogue2M().to(device)
    ...
    for iter in range(MAX_ITERS):
        ...
        if iter % EVAL_INTERVAL == 0 or iter == MAX_ITERS - 1:
            ...
            ckpt_path = f"weights/dialogue_2m_iter{iter:05d}_val{losses['val']:.4f}.bin"
            export_weights(model, ckpt_path)
            print(f"  -> Checkpoint: {ckpt_path}")

            if losses['val'] < best_val_loss:
                best_val_loss = losses['val']
                export_weights(model, "weights/arianna_dialogue_2m_best.bin")
                print(f"  -> New best! Also saved to arianna_dialogue_2m_best.bin")
    ...
    # NOTE: Best weights already saved during training, don't overwrite with final (worse) weights
    print("Best weights already saved at weights/arianna_dialogue_2m.bin")
