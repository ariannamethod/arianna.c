...
// zig 0.16 removed std.time.microTimestamp; read micros straight from libc
// (vagus links libc anyway). Monotonic clock — fine for signal freshness/order.
const ctime = @cImport({
    @cInclude("time.h");
});
fn nowMicros() i64 {
...
pub const Signal = packed struct {
    ...
    pub fn now(source: Source, signal_type: SignalType, value: f32) Signal {
        return .{
            ...
            .value = value,
            .timestamp_us = @intCast(nowMicros()),
        };
    }
};
...
pub const VagusNerve = struct {
    ...
    pub fn tick(self: *Self) ?Signal {
        const now = @as(u64, @intCast(nowMicros()));
